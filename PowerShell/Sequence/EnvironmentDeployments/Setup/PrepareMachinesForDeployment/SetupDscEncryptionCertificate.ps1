...
# This script is a copy of several cmdlets to setup a certificate on machines with PowerShell 4.0.
# The actual work is done at the bottom of this script, simply being 'Install-DscEncryptionCertificate'
...
Function Get-DscEncryptionCertificate {
	<#
	.SYNOPSIS
	Creates a self-signed encryption certificate for encrypting MOF files created from DSC configurations.
	#>
	[CmdletBinding()]
	param (
		[string]
		$CommonName = 'DscEncryption'
	)
	process {
		Get-ChildItem Cert:\LocalMachine\My `
				 | ?{ $_.Subject -eq "CN=$CommonName" -and $_.NotBefore -le (Get-Date) -and $_.NotAfter -ge (Get-Date) } `
...
function ExportCertificate {
    ...
    if (!(Test-Path $FilePath.FullName)) {
        New-Item $FilePath.FullName -ItemType file
    }
    [io.file]::WriteAllBytes($FilePath.FullName, $certs.Export('cer'))
}

Function Install-DscEncryptionCertificate {
	<#
	...
	The common name of the certificate to create.

	.PARAMETER ExportCertificateToTempFile
	Exports the certificate to a temporary file, returning an hashtable with the keys File, FilePath and Thumbprint.
	...
	param (
		...
		$CommonName = 'DscEncryption',
		[switch]
		$ExportCertificateToTempFile
	)
	begin {
		$ErrorActionPreference = "Stop"
	}
	process {
		$cert = Get-DscEncryptionCertificate -CommonName $CommonName
		...
		if ($cert -eq $null) 
		{
			...
			if ($cert -eq $null) {
				throw "The encryption certificate with common name '$commonName' cannot be found"
				return
			}

			$certExportFilePath = "$env:temp\DscEncryptionPublicKey.cer"

			# Export the public key certificate
			ExportCertificate -Cert $cert -FilePath $certExportFilePath | Out-Null

			# Import the public certificate into the trusted root.
			certutil -addstore "Root" "$certExportFilePath" | Out-Null
		}
        
		if ($ExportCertificateToTempFile) {

			# Create a new temporary file and export the certificate to it.
			$certFilePath = (Get-Item ([System.IO.Path]::GetTempFileName())).FullName

			ExportCertificate -Cert $cert -FilePath $certFilePath | Out-Null
...
# This does the work!
$certRequired = (Get-DscEncryptionCertificate) -eq $null
if ($certRequired) {
	Install-DscEncryptionCertificate
}
