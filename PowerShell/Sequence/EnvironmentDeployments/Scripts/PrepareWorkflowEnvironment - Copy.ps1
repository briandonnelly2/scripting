...
$serverConfigFileLocation = $PSScriptRoot + "\Servers.environmentConfig"
$packageConfigFileLocation = $PSScriptRoot + "\Packages.environmentConfig"

$processStopped = $false
$processStarted = $false

Try {
#region read configs, set variables
...
#endregion read configs, set variables

	$processStarted = $true
	
	# Stopping order (default is starting)
		...
		foreach ($serverConfig in $serverConfigs) {
			Try {
				...
				#region remote execution
					...
					Try {
						#region Read node config
						$ErrorActionPreference = "Stop"
						
						$environmentFileLocation = "Environment.deploymentConfig"
						#$windowsServicePrefix = "KPMG.TaxConnect."
						$windowsServiceSuffix = "Service"
						...
						#endregion Read node configs
						
						#region BRS ---------------------------------------------------------------
						
						if ($area -eq "Background Runtime Services") {
							$brsServices = Get-Service * | Where-Object {$_.Name -like "$brsServiceName*"}

							foreach($brsService in $brsServices) {
								$doneSomething = $true
								if ($stopping -eq $true) {
									# BRS Stopping
									if ($brsService.Status -eq "Running") {
										Write-Output ("Stopping " + $brsService.Name + " on " + $serverConfig.Server + "...")
										Stop-Service $brsService.Name
										Write-Output ("    " + $brsService.Name + " is stopped.")
									} elseif ($brsService.Status -eq "Stopped") {
										Write-Output ($brsService.Name + " is stopped.")
									} else {
										Write-Output ($brsService.Name + " is not in an an expected 'Running' state on - the current state is " + $brsService.Status)
									}
								} else {
									# BRS Starting
									if ($brsService.Status -eq "Stopped") {
										Write-Output ("Starting " + $brsServic.Name + " on " + $serverConfig.Server + "...")
										Start-Service $brsService.Name
										Write-Output ("    " + $brsService.Name + " is running.")
									} elseif ($brsService.Status -eq "Running") {
										Write-Output ($brsService.Name + " is running.")
									} else {
										Write-Output ($brsService.Name + " is not in an expected 'Stopped' state on - the current state is " + $brsService.Status)
									}
								}
							}
						}
						...
						foreach($package in $serverPackages) {
							...
							$isWindowsService = $package.PackageName.EndsWith($windowsServiceSuffix)
							...
							#region Web Applications / Services ---------------------------------------------------------------
							...
							#endregion Web Applications / Services ---------------------------------------------------------------
						}
