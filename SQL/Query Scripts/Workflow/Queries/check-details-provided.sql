...
/* ----------------------------------------------------------- */
/* A. REQUIRED INPUTS: The information provided to us on the setup sheet */
/* You need to enter this data for the script to return valid  */
/* Information.  Missing details should be obtained from the   */
/* Processiong Team if required.                               */
/* ----------------------------------------------------------- */
...
ELSE
BEGIN
	PRINT N'The MSD/SAP Lead Client with code, ' + @ReturnedClientNumber + ', already exists on workflow.'
	PRINT N'This has a workflow ID of ' + @LeadClientID + ' and is named ' + @WFLeadClientName
	PRINT N''
END
...
BEGIN
	...
	PRINT N'If setupo by you, proceed with creating the client in workflow'
	PRINT N''
END
--2. If the supplied Atlas ID DOES NOT matche an existing client in DPM...
ELSE
BEGIN
	PRINT N'A client with these details does not yet exist in Digita.  This should be created in DPM by selecting "New Company CLient" and using ' + @AtlasID + ' as the clients Digita code.'
	PRINT N'Once complete, please run this script again to obtain the billable entity ID for client setup in workflow.'
	PRINT N''
END