...
DECLARE @MSDOppNumber NVARCHAR(100) = '90034806' --SAP/MSD opportunity number
DECLARE @AtlasID NVARCHAR(100) = '41277' --Atlas ID
DECLARE @MSDClientNumber NVARCHAR(100) = '60622282' --SAP/MSD client number
DECLARE @SuppliedClientName NVARCHAR(100) = 'Opel Ireland' --The supplied client name
...
ELSE IF (@CanGoOffshore = 'True/False') 
	BEGIN PRINT N'@CanGoOffshore value has not been chosen. Chose either True or False, not both and re-run. Terminating execution.' SET NOEXEC ON END
ELSE
...
BEGIN PRINT N'The MSD/SAP Lead Client number provided on the setup sheet, ' + @MSDClientNumber + ', does not match the client number that is found, ' + @ReturnedClientNumber + '.'
...
BEGIN PRINT N'The MSD/SAP Lead Client number provided on the setup sheet, ' + @MSDClientNumber + ', matches the code that is found, ' + @ReturnedClientNumber + ' and is named ' + @MSDLeadClientName + '.'
	  PRINT N'' END
--3. Else the supplied SAP/MSD opportunity code returns no results from the extract database.
ELSE 
BEGIN PRINT N'The supplied opportunity ID of: ' + @MSDOppNumber + ' does not match any existing SAP/MSD engagements'
...
BEGIN PRINT N'The MSD/SAP Lead Client with code, ' + @ReturnedClientNumber + ', and name ' + @MSDLeadClientName + ' does not exist on workflow yet.'
	  ...
	  PRINT N'' END
--2. Else if a primary SAP link IS NOT found using the returned client number AND the name starts with KPMG...
ELSE IF (@LeadClientID IS NULL AND @MSDLeadClientName LIKE 'KPMG%')
BEGIN PRINT N'The MSD/SAP Lead Client with code, ' + @ReturnedClientNumber + ', and name ' + @MSDLeadClientName + ' does not exist on workflow yet.'
	  PRINT N'Ask the processing team if we need to setup this lead client as a seperate entity from the one we are being asked to setup'
	  PRINT N'' END
--3. Else a primary SAP link IS found using the returned client number...
ELSE
BEGIN PRINT N'The MSD/SAP Lead Client with code, ' + @ReturnedClientNumber + ', already exists on workflow. This has a workflow ID of ' + @LeadClientID + ' and is named ' + @WFLeadClientName + '.'
	  PRINT N'' END
...
BEGIN PRINT N'A client already exists on workflow with the workflow client ID of ' + @ClientID + ' and is named ' + @WFClientName + '.'
	  PRINT N'' END
--2. If there IS NOT a match for any existing entries in the links table...
ELSE
BEGIN PRINT N'A client with these details does not yet exist on workflow.  This can be setup in workflow provided all other details check out.'
	  PRINT N'' END
...
BEGIN PRINT N'The Atlas ID provided in the setup sheet, ' + @AtlasID + ', matches an existing client in Digita with billable entity ID, ' + @BBEID + ' and client name of, ' + @DigitaClientName 
...
ELSE
BEGIN PRINT N'A client with these details does not yet exist in Digita.  This should be created in DPM by selecting "New Company Client", using ' + @AtlasID
	  PRINT N'as the clients Digita code, ' + @SuppliedClientName + ' as the clients name. As the supplied offshore value is ' + @CanGoOffshore 
		    + ', ensure this client is set to the ' + @ClientGroupName + ' client access group.'
