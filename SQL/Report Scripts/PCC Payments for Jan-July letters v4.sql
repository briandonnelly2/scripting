...
(
	...
	where wi.BusinessServiceLineId = 2
...
(
	...
	inner join [DIGITATAX].PracticeManagementCoE.dbo.listcache lc on lc.billableentityid = be.billableentityid
	inner join [DIGITATAX].PracticeManagementCoE.dbo.practiceclient pc on pc.practiceclientid = lc.practiceclientid
	inner join [DIGITATAX].PracticeManagementCoE.dbo. person p on p.personid = lc.personid
	left join [DIGITATAX].PracticeManagementCoE.dbo.postalcontact posc on lc.postalcontactid = posc.postalcontactid
	left join [DIGITATAX].PracticeManagementCoE.dbo.[Country] cou on cou.[CountryID] = posc.[CountryID]
	left join [DIGITATAX].PracticeManagementCoE.dbo.practiceperson pp on p.personid = pp.personid
	left join [DIGITATAX].PracticeManagementCoE.dbo.emailcontact email on lc.emailcontactid = email.emailcontactid
	left join [DIGITATAX].PracticeManagementCoE.dbo.PracticePerson pps ON lc.ClientSignatoryID = pps.PersonID
	left join [DIGITATAX].PracticeManagementCoE.dbo.PracticeUser pu ON pps.PracticePersonID = pu.PracticePersonID
	left join [DIGITATAX].PracticeManagementCoE.dbo.Employment emp ON pu.EmploymentID = emp.EmploymentID
	left join [DIGITATAX].PracticeManagementCoE.dbo.KPMG_TrackerReporting_UDFs udf on udf.billableentityidudf = be.billableentityid 
	left join [DIGITATAX].PracticeManagementCoE.dbo.KPMG_TrackerReporting_Relationships rel on rel.billableentityidrel = be.billableentityid
...
(
	...
	from  [DIGITATAX].TaxywinCoE.dbo.client as c 
	inner join [DIGITATAX].TaxywinCoE.dbo.paymentinstalment as pi2 on c.CLIENTID = pi2.ClientID
	left join [DIGITATAX].TaxywinCoE.dbo.paymentadjustment pa on pa.installmentid = pi2.installmentid
...
(
	...
	from  [DIGITATAX].TaxywinCoE.dbo.client as c 
	inner join [DIGITATAX].TaxywinCoE.dbo.otherreturninfo as ori on c.CLIENTID = ori.lClientID
...
(
	...
	from  [DIGITATAX].TaxywinCoE.dbo.client as c 
		LEFT JOIN [DIGITATAX].TaxywinCoE.dbo.[Residence] r WITH (NOLOCK) ON c.[CLIENTID] = r.[lClientID] AND r.[nYear] = @TaxYear
		LEFT JOIN [DIGITATAX].TaxywinCoE.dbo.[CRSA100] crs WITH (NOLOCK) ON c.[CLIENTID] = crs.[ClientID] AND crs.[TaxYear] = @TaxYear
		LEFT JOIN [DIGITATAX].TaxywinCoE.dbo.[CRTaxRates] crtr WITH (NOLOCK) ON c.[CLIENTID] = crtr.clientid and crtr.taxyear = @TaxYear
	where 
		crs.[TaxYear] = @TaxYear
),
...
left join DPTClientCode dpt on dpt.entityid = pr.foreignid COLLATE Latin1_General_CI_AS_KS_WS
        ...
        LEFT JOIN (
                ...
                FROM
                        [DIGITATAX].TaxywinCoE.dbo.Client c
                        LEFT JOIN [DIGITATAX].TaxywinCoE.dbo.PaymentInstalment payi ON c.ClientID = payi.ClientID
                        LEFT JOIN [DIGITATAX].TaxywinCoE.dbo.PaymentAdjustment paya ON payi.InstallmentID = paya.InstallmentID
                WHERE
                        (payi.[Year] = @TaxYear + 1)
