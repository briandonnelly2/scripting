...
--V 2.0 - Amended report sent by Digita after business said this was not pulling what they needed.  please see Lynsey Tait for details
--V 2.1 - Added ability to filter by client group (PCC, GMS etc) at the bottom of the script.
...
FROM Client C 
	...
	--Pre Owned Benefit
			...
			FROM Income
			WHERE  [TYPE] = 'PREOWNED' AND PL_PClientid IS NULL
	...
	--gift aid
			...
			FROM OtherOut
			WHERE TYPE = 'GAIDCOV' --AND PL_PClientid IS NULL
	...
	--charity assets
			...
			FROM OtherOut
			WHERE TYPE = 'GIFTCHAR' --AND PL_PClientid IS NULL
	...
	--life assurance gains
			...
			FROM Income
			WHERE  TYPE = 'LFEASSGN' AND PL_PClientid IS NULL
	...
	--other reliefs
			...
			FROM OtherOut
			WHERE TYPE IN ('EISSCREL','VNTCPTRS') AND PL_PClientid IS NULL
	...
	--CGT gains
		( SELECT ClientID, Year, SUM(ISNULL(QuotedDisposals,0)) AS QuotedDisposals ,SUM(ISNULL(QuotedGains,0)) AS QuotedGains ,SUM(ISNULL(QuotedLosses,0)) AS QuotedLosses ,
		 ...
		 SELECT [CLIENTID]  ,YEAR ,SUM(CASE WHEN [TOTALGAIN] > 0 THEN [TOTALGAIN] ELSE 0 END) AS QuotedGains
		  ...
		  FROM [CGT] 
		  WHERE nCategory = 1 AND ISNULL(cERAmountClaimed,0) = 0 AND ISNULL(cDeferredERAmount,0)  = 0 --AND PL_PClientid IS NULL
		 ...
		 SELECT [CLIENTID]  ,YEAR ,SUM(CASE WHEN [TOTALGAIN] > 0 THEN [TOTALGAIN] ELSE 0 END) AS UnquotedGains
		  ...
		  FROM [CGT]
		  WHERE nCategory = 2 AND ISNULL(cERAmountClaimed,0) = 0 AND ISNULL(cDeferredERAmount,0)  = 0 --AND PL_PClientid IS NULL
		  ...
		  FROM [CGT]
		  WHERE nCategory = 13 AND ISNULL(cERAmountClaimed,0) = 0 AND ISNULL(cDeferredERAmount,0)  = 0 --AND PL_PClientid IS NULL
		  ...
		  FROM [CGT]
		  WHERE nCategory = 4 AND ISNULL(cERAmountClaimed,0) = 0 AND ISNULL(cDeferredERAmount,0)  = 0 --AND PL_PClientid IS NULL
		  ...
		  FROM [CGT]
		  WHERE nCategory = 5 OR ISNULL(cERAmountClaimed,0) != 0 OR ISNULL(cDeferredERAmount,0)  != 0 --AND PL_PClientid IS NULL
...
FROM CGT LEFT JOIN
		...
		FROM (SELECT [ID], MAX(ACQEVENT) AS MaxRelief , COUNT(ACQEVENT)  AS CntRelief
				...
				WHERE bNegligibleValueClaim = 1 OR 
						...
						CGTA.ClaimCode IS NOT NULL --AND PL_PClientid IS NULL
) C) P
PIVOT (MAX(ClaimCode) FOR Category IN([Quoted],[Unquoted],[Residential],[Other])) AS pvt) CGTClaims ON TF.CLIENTID = CGTClaims.CLIENTID AND TF.Year = CGTClaims.YEAR
	...
	(SELECT lClientID, nYear, SUM(ISNULL(cNEtAmount,0)) AS NetBanks
		...
		WHERE lTypeID IN (SELECT lTypeID FROM NewInterestTypeRef where bGrossFrom2017	= 0)
				...
				AND bUnderwriting = 0 AND PL_PClientid IS NULL
	...
	(SELECT lClientid, nYear, SUM(ISNULL(UTP.cNetAmount,0)) AS NetUT
		...
		WHERE UT.bInterestPaidGross = 0  AND lPartnershipId IS NULL
	...
	(SELECT lClientid, nYear, SUM(ISNULL(UTP.cGrossAmount,0)) AS GrossUT
		FROM NewInterestUT UT INNER JOIN NewInterestUTPayments UTP ON UT.lUnitTrustId = UTP.lUnitTrustId
		WHERE UT.bInterestPaidGross = 1  AND lPartnershipId IS NULL
	...
	(SELECT ClientID, year, SUM(ISNULL(GrossAmount,0)-ISNULL(TaxFreeAmount,0)) As ForeignInt
		...
		WHERE [Type] = 'FRGBAOI' AND ISNULL(TaxDedUk,0) = 0 AND ISNULL(FrgnTaxDeducted,0)= 0 AND bUnderwriting = 0 AND PL_PClientid IS NULL
	...
	(SELECT CLIENTID, YEAR, SUM(ISNULL(cDivTotalValue,0)) AS ForeignDiv
		FROM OtherForeignIncome
		WHERE [Type] = 'OFRGNDIV' AND ISNULL(TaxDedUk,0) = 0 AND ISNULL(FrgnTaxDeducted,0)= 0 AND bUnderwriting = 0 AND PL_PClientid IS NULL
...
WHERE bInvestmentTrust = 0 AND lTypeId  IN(10,90) AND lPartnershipId IS NULL
GROUP BY lClientid, nYear) UD ON UD.lClientid = TF.CLIENTID AND UD.nYear = TF.YEAR LEFT JOIN
	...
	(SELECT lClientid, nYear, SUM(ISNULL(cGrossAmount,0)) AS OtherDividends
		FROM NewShare NS INNER JOIN NewDividend ND ON NS.lShareId = ND.lShareId
		WHERE bInvestmentTrust = 1 OR lTypeId IN (60,70) AND lPartnershipId IS NULL
	...
	(SELECT ClientID, Year FROM 
		(  SELECT DISTINCT ClientID, Year
			  FROM CGT
			  WHERE Estimate = 1 --AND PL_PClientid IS NULL
			  UNION 
			  SELECT DISTINCT ClientID, Year
			  FROM CapitalGainsSummary
...
AND gc.GroupId IN ('PCC Offshore','PCC UK')