...
SELECT   wid.[WorkItemId] AS 'WorkItem ID'
		...
		,rn.Comment
		--,d.DocumentId
		--,d.WorkSiteVersionNumber
		--,r.CreatedTimeStamp
		--,rs.SubmittedBy
		--,rs.SubmittedTimeStamp
  FROM [TaxWFPortalData].[dbo].[WorkItemDetail] AS wid

  LEFT JOIN [Sequence]..[UACT9fb7f569f014441cb9d8b966ee165672] AS uact ON uact.WorkItemInstanceId = wid.SequenceInstanceId
  LEFT JOIN [TaxWFReviewToolData]..[ReviewSession] AS r ON r.ExternalId = uact.ExternalId
  INNER JOIN [TaxWFReviewToolData]..[ReviewStep] rs ON r.ReviewStepId = rs.Id
  INNER JOIN [TaxWFReviewToolData]..[User] u ON u.Id = r.UserId
  INNER JOIN [TaxWFReviewToolData]..[DocumentVersion] d ON d.Id = rs.DocumentVersionId
  LEFT JOIN [TaxWFReviewToolData]..[ReviewNote] AS rn ON rn.CreatedReviewStepId = r.ReviewStepId
  ...
  ORDER BY wid.[WorkItemId], r.CreatedTimeStamp Desc
...
SELECT R.[DocumentId], Position AS [ReviewPoint No], CASE WHEN R.Id = 0 THEN 'ReviewPoint' ELSE 'Response' END AS [Item Type], 
		[Type] AS [Category], 
