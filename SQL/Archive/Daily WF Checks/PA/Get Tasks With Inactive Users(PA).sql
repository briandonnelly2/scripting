/**
	This script will return any tasks that are with inactive users in partner's workflow.
	We need to find out what should be done with them by sending these on an excel spreadsheet 
	to Julie Simpson. These will usually either be assigned to someone else or cancelled.
...
SELECT 
	...
	ai.fldSubject, v.WorkItemId, wi.WorkItemStateId
FROM
	tblActionItems ai
JOIN 
	tblEmployees e on ai.fldToId = e.fldEmployeeId and e.fldActive = 0
JOIN
	tblInstanceActivities ia on ai.fldIActId = ia.fldId
JOIN
	tblInstanceWorkflows iw on ia.fldInstanceWfId = iw.fldid and iw.fldstatus NOT IN(7) and iw.fldCompletionDate is null
JOIN
	tblInstanceWorkflowsDataPropagation v on ia.fldInstanceWfId = v.fldIWfId
LEFT JOIN
	TaxWFPortalDataPA.dbo.WorkItems wi on v.WorkItemId = wi.WorkItemId 	
WHERE
	ia.fldID IN
	(
		SELECT
		...
		(
			...
			SELECT 
				ai.fldIActId Id, SUM(CONVERT(INT,e.fldActive)) s , Count(e.fldEmployeeId) c
			FROM
				tblActionItems ai 
			JOIN
				tblEmployees e on e.fldEmployeeId = ai.fldToId
			WHERE
				ai.fldIActId IS NOT NULL
			GROUP BY ai.fldIActId
			HAVING
			(SUM(CONVERT(INT,e.fldActive)) = Count(e.fldEmployeeId))
			OR
			(SUM(CONVERT(INT,e.fldActive)) = 0)
		) d
	) and
...
and v.workitemid not in
	(
	0
	)
and ISNULL(wi.workitemstateid,1) = 1
